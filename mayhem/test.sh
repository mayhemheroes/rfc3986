#!/usr/bin/env bash
#
# mayhem/test.sh — RUN rfc3986's own pytest suite (deps already installed by mayhem/build.sh) and
...
#
# It does NOT compile — build.sh installed pytest + idna + rfc3986 into the in-image site dir and
# compiled the rfc3986_run_tests ELF wrapper. We only RUN the suite. The suite is routed through that
...
# Put the in-image site dir (atheris + pytest + rfc3986 .pth) on PYTHONPATH.
